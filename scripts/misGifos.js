        ...
        fetch(`https://api.giphy.com/v1/gifs?api_key=${Key_api}&ids=${localStorage.getItem('misGifos')}`).then((objetoMisGifos) => {
            ...
            .then((dataInfo) => {
                ...
                for (let i = 0; i < dataInfo.data.length; i++){
                    ...
                    allGifs.push(new Gif (dataInfo.data[i].images.downsized_medium.url, dataInfo.data[i].images.downsized_medium.url, dataInfo.data[i].id, dataInfo.data[i].title, dataInfo.data[i].username));                    
                }    
            });
        });
    }  
